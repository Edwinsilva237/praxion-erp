...
router.get('/delivery-notes', checkPermission('sales', 'read'), async (req, res, next) => {
  try {
    const { type, status, partnerId, from, to, search, sortBy, sortDir, page, limit, invoiceable } = req.query
    const result = await deliveryNoteService.listDeliveryNotes({
      tenantId: req.tenant.id,
      type: type || 'sale', status, partnerId, from, to, search, sortBy, sortDir,
