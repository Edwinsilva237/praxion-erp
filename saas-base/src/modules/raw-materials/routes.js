...
router.post('/', checkPermission('products', 'create'), async (req, res, next) => {
  try {
    const { name, resinType, materialType, itemKind, unit, maxRegrindPct, costPerKg, description, leadTimeDays } = req.body
    ...
    const item = await svc.createRawMaterial({
      tenantId: req.tenant.id,
      name, resinType, materialType, itemKind: kind, unit, maxRegrindPct, costPerKg, description, leadTimeDays,
      userId: req.auth.userId, ipAddress: req.ip, userAgent: req.get('user-agent'),
    })
    res.status(201).json(item)
  } catch (err) {
    if (err.code === '23505') return res.status(409).json({ error: 'Ya existe una materia prima con ese nombre.' })
    next(err)
  }
})
...
router.patch('/:id', checkPermission('products', 'update'), async (req, res, next) => {
  try {
    const { name, materialType, unit, maxRegrindPct, costPerKg, description, isActive, leadTimeDays } = req.body
    const item = await svc.updateRawMaterial({
      tenantId: req.tenant.id, id: req.params.id,
      name, materialType, unit, maxRegrindPct, costPerKg, description, isActive, leadTimeDays,
      userId: req.auth.userId, ipAddress: req.ip, userAgent: req.get('user-agent'),
    })
    if (!item) return res.status(404).json({ error: 'Materia prima no encontrada.' })
    res.json(item)
