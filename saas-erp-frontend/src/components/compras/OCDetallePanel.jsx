...
// ── Modal envío por correo al proveedor (multi-destinatario + PDF adjunto) ──
function OCEmailModal({ oc, onSend, onClose, sending }) {
  ...
  const { data: contactsData, isLoading } = useQuery({
    queryKey: ['purchase-order', oc.id, 'contacts'],
    queryFn:  () => purchasesApi.orderContacts(oc.id),
    staleTime: 30_000,
  })
  ...
  const extraList = extraEmails.split(/[\s,;]+/).map(e => e.trim()).filter(Boolean)
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/40 p-4">
      <div className="card w-full max-w-md p-5">
        <h3 className="text-base font-semibold text-ink-primary mb-1">
          Enviar OC {oc.order_number} por correo
        </h3>
        <p className="text-xs text-ink-muted mb-4">
          Se enviará el PDF de la orden de compra a los contactos seleccionados del proveedor.
        </p>

        {oc.status === 'draft' && (
          <div className="mb-3 bg-status-warning/10 border border-status-warning/40 rounded-lg px-3 py-2">
            <p className="text-xs text-status-warning">
              La OC está en borrador: al enviarla se <strong>confirmará</strong> (pasará a
              &quot;Enviada&quot;) y ya no será editable.
            </p>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="bg-status-warning/10 border border-status-warning/40 rounded-lg px-3 py-2">
            <p className="text-xs text-status-warning">
              {oc.partner_id
                ? 'Este proveedor no tiene contactos con correo. Agrega uno abajo o regístralo en el catálogo de proveedores.'
                : 'OC con proveedor genérico (fuera del catálogo). Escribe abajo el correo del proveedor.'}
            </p>
          </div>
        )}
...
function AccionesOC({ oc, onAction, loadingAction }) {
  ...
  const EmailBtn = () => (
    <Btn label="Enviar correo" action="email"
...
export function OCDetallePanel({ ocId, onClose, onGoToRecepcion }) {
  ...
  // Envía la OC por correo. Si estaba en borrador, el backend la confirma
  // primero — de ahí la invalidación de inventario en tránsito al terminar.
  async function handleSendEmail(emails) {
    setError(null)
    setMsg(null)
    setSendingEmail(true)
    const wasDraft = oc?.status === 'draft'
    try {
      const res = await purchasesApi.sendOrderEmail(ocId, { emails })
      setShowEmail(false)
      setMsg(`OC enviada a ${res?.email?.recipients?.join(', ') || 'los destinatarios'}.`)
      ...
      // El backend pudo haber confirmado el borrador aunque el correo fallara.
      qc.invalidateQueries({ queryKey: ['purchase-order', ocId] })
      qc.invalidateQueries({ queryKey: ['purchase-orders'] })
      setError(e.response?.data?.error || e.message || 'No se pudo enviar el correo')
    } finally {
        ...
        <div className="flex-1 p-5">
          {isLoading ? (
            ...
            <>
              ...
              {actionMsg && (
                <div className="mt-4 bg-status-success/10 border border-status-success/40 rounded-lg px-3 py-2">
                  <p className="text-sm text-status-success">{actionMsg}</p>
                </div>
              )}
