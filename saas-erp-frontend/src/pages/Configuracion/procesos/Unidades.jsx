...
const UNIT_TYPES = [
  ...
  { value: 'count',    label: 'Unidades (conteo)' },
...
const EMPTY_FORM = {
  code: '', name: '', symbol: '', unit_type: 'weight', is_base: false, decimals: 3, sort_order: 0,
}

function UnitModal({ item, onClose, onSaved }) {
  const isNew = !item?.id
  const [form, setForm] = useState(() => isNew
    ? { ...EMPTY_FORM }
    : { code: item.code, name: item.name, symbol: item.symbol, unit_type: item.unit_type, is_base: item.is_base ?? false, decimals: item.decimals ?? 3, sort_order: item.sort_order ?? 0 }
  )
  const [error, setError] = useState(null)

  const mut = useMutation({
    mutationFn: isNew
      ? () => processConfigApi.createUnit(form)
      : () => processConfigApi.updateUnit(item.id, form),
    onSuccess: () => onSaved(),
        ...
        <div className="p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input className="input" placeholder="ej: kg" value={form.code} onChange={e => set('code', e.target.value)} disabled={!isNew} />
            </div>
            <div>
              <label className="label">Símbolo</label>
              <input className="input" placeholder="ej: kg" value={form.symbol} onChange={e => set('symbol', e.target.value)} />
            </div>
          </div>
          <div>
            <label className="label">Nombre</label>
            <input className="input" placeholder="ej: Kilogramo" value={form.name} onChange={e => set('name', e.target.value)} />
          </div>
          <div>
            <label className="label">Tipo</label>
            <select className="select" value={form.unit_type} onChange={e => set('unit_type', e.target.value)} disabled={!isNew}>
          ...
          <label className="flex items-center gap-3 text-sm cursor-pointer">
            <input type="checkbox" checked={form.is_base} onChange={e => set('is_base', e.target.checked)} className="w-4 h-4 accent-brand-600" disabled={!isNew} />
            <span className="font-medium text-ink-primary">Unidad base de su tipo</span>
          </label>
        </div>
...
export default function Unidades() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ['process-config-units', showInactive],
    queryFn:  () => processConfigApi.listUnits({ include_inactive: showInactive || undefined }),
  })

  const toggleMut = useMutation({
    mutationFn: ({ id, isActive }) => processConfigApi.updateUnit(id, { is_active: isActive }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['process-config-units'] })
    ...
    <div className="page-enter flex flex-col gap-6">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Catálogo de unidades usadas en recetas, inventario y producción</p>
        </div>
        {canManage && (
      ...
      {isLoading ? (
        ...
        <div className="empty-state">
          <p className="font-medium text-ink-secondary">Sin unidades configuradas</p>
          {canManage && <button onClick={() => setEditing('new')} className="btn-primary btn-sm mt-3">+ Crear primera</button>}
        </div>
      ) : (
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th>Símbolo</th>
                <th>Tipo</th>
                <th className="text-right">Decimales</th>
                <th>Base</th>
                <th>Estado</th>
            ...
            <tbody>
              {items.map(item => (
                <tr key={item.id} className={clsx(!item.is_active && 'opacity-50')}>
                  ...
                  <td className="font-mono text-sm">{item.symbol}</td>
                  <td className="text-sm text-ink-secondary">{typeLabel(item.unit_type)}</td>
                  <td className="text-right font-mono text-sm">{item.decimals}</td>
                  <td><Badge variant={item.is_base ? 'blue' : 'gray'} label={item.is_base ? 'Sí' : 'No'} /></td>
                  <td><Badge variant={item.is_active ? 'green' : 'gray'} label={item.is_active ? 'Activa' : 'Inactiva'} /></td>
                  {canManage && (
                    <td>
      ...
      {editing && (
        <UnitModal item={editing === 'new' ? null : editing} onClose={() => setEditing(null)} onSaved={handleSaved} />
      )}
    </div>
  )
}
